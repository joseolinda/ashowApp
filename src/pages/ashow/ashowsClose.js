...
export default class AShowsClose extends Component {

    static navigationOptions = {
        title: "AShows Finalizadas",
        headerStyle: {
    ...
    loadAShows = async (page = 1) => {
        try {
            const response = await api.get(`/occurrences/searchUserClose?page=${page}`);

            console.log(response);

            this.setState({
                ashows: response.data.data,
                page,
